...
rho_var[, median_cbg_area := median_cbg_area_km2]
...
rho_var_county[, median_cbg_area := median_cbg_area_km2]
# travel mode
...
vars_A <- c("pop", "area", "median_cbg_area")
...
vars_A_z <- c("pop_z", "area_z", "median_cbg_area_z")
...
fit_A_county <- lmer(rho ~ pop_z + area_z + median_cbg_area_z + factor(day) + (1 | msa),
                     data = dat_county, REML = TRUE)
fit_B_county <- lmer(rho ~ mean_haversine_z + drive_alone_z + public_transit_z + factor(day) + (1 | msa),
                     data = dat_county, REML = TRUE)
fit_AB_county <- lmer(rho ~ pop_z + area_z + median_cbg_area_z +
                        mean_haversine_z + drive_alone_z + public_transit_z +
                        factor(day) + (1 | msa),
                      data = dat_county, REML = TRUE)

#----------MSA: block-wise contribution----------#
fit_A_msa <- lmer(rho ~ pop_z + area_z + median_cbg_area_z + factor(day) + (1 | msa),
...
dd_cty <- dredge(
  ...
  rank = "AICc",
  trace = FALSE,
  subset = !(pop_z & area_z & median_cbg_area_z)
)
...
ggsave(fig.rho.var.r2, filename = paste(figpath,"/msa/SI_rho_var_r2_",Yname[yindex],".pdf",sep=""), width =4*4, height = 4.5*1)
