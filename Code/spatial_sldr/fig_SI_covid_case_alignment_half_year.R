...
#----------confirmed cases for COVID-19: new date of 5 weeks before + 5 weeks during----------#
...
write.csv(cases,file=paste(datapath.dis,"/us-msas-daily-cumulative-case_half_year.csv",sep=""),row.names = FALSE)




#----------percent change of median rho and confirmed cases for COVID-19----------#
con.msa <- read.csv(paste(datapath.dis,"/us-msas-daily-cumulative-case_half_year.csv",sep=""), header=TRUE)
...
for(yindex in 2:2){
  ...
  # daily rho
  rho.mob.msa <-fread(paste0(datapath.msa, "/msa/SLDR_params_", Yname[yindex], "_half_year.csv"))
  rho.mob.msa <- rho.mob.msa[day %in% Datevalue]
  
  rho.mob.msa <- read.csv(paste(datapath.msa,"/msa/SLDR_params_",Yname[yindex],"_half_year.csv",sep=""), header=TRUE)%>%setDT
  rho.mob.msa <- rho.mob.msa[day %in% Datevalue]
  ...
  A.msa[[yindex]] <- ggplot(rho.case.msa, aes(x = percent_new_case, y = percent_det_rho)) +
  ...
  #----------plot the percent change of rho and cum confirmed cases----------#
  # --- after you create rho.case.msa and define percent_case / percent_new_case ---
  rho.case.msa$log_percent_case <- log10(rho.case.msa$percent_case)
  
  # ------------------------------------------------------------
  # Bin x (severity) and average y within bins, then compute correlation
  # ------------------------------------------------------------
  # Option A (recommended): equal-width bins in log10-space (stable on log x-axis)
  nbin <- 8  # you can tune (e.g., 6–12)
  bin_breaks <- seq(min(rho.case.msa$log_percent_case, na.rm = TRUE),
                    max(rho.case.msa$log_percent_case, na.rm = TRUE),
                    length.out = nbin + 1)
  
  rho.case.bin <- as.data.table(rho.case.msa)
  rho.case.bin[, xbin := cut(log_percent_case, breaks = bin_breaks,
  ...
  rho.case.bin_sum <- rho.case.bin[!is.na(xbin), .(
    ...
    n          = .N
  ), by = xbin][n >= 3]   # optional: drop sparse bins
  ...
  # If you prefer to report/plot x back on original scale:
  rho.case.bin_sum[, x_mean := 10^x_log_mean]
  
  # ------------------------------------------------------------
  # (Optional) Use binned correlation for the annotation in your original scatter plot
  # ------------------------------------------------------------
  pearson.cor <- pearson.cor.bin
  
  # ------------------------------------------------------------
  # (Optional) Add binned-mean points/line on top of the original scatter
  # ------------------------------------------------------------
  B.msa[[yindex]] <- ggplot(rho.case.msa, aes(x = percent_case, y = percent_det_rho)) +
    geom_point(aes(fill = msa, color = msa, shape = msa), size=3, stroke=0.5) +
    # binned mean trend (black overlay; no need to map aesthetics)
    geom_point(data = rho.case.bin_sum, aes(x = x_mean, y = y_mean),
               inherit.aes = FALSE, shape = 21, size = 2.8, stroke = 0.6, fill = "white", color = "black") +
    geom_line(data = rho.case.bin_sum, aes(x = x_mean, y = y_mean),
    ...
    theme(panel.border = element_rect(fill=NA,color="black", linewidth=0.5, linetype="solid"),
          ...
          legend.key = element_blank()) +
    annotation_custom(
      grob = textGrob(TeX(sprintf("$Pearson's\\,r = %.2f$", round(pearson.cor, 2))),
                      gp = gpar(fontsize = 12), hjust = 0),
      xmin = log(xrange[2]/7,10), xmax = log(xrange[2]/7,10),
      ymin = 0.018, ymax = 0.02
    )
  
  
  #----------7-day average new cases/population----------#
